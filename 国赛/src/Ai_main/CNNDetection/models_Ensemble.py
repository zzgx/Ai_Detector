...
# CUDA_VISIBLE_DEVICES=0 python CNNDetection/models_Ensemble.py --dataroot latent_diffusion
...
# 替换单一模型的加载为加载多个模型
model_paths = [
               "CNNDetection/model/tag_testdata_B_C1_epoch_10_acc_100.0.pth",
               "CNNDetection/model/B_C1_plan1_epoch_8_acc_97.75.pth",
               "CNNDetection/model/B_C1_plan2_epoch_10_acc_97.75.pth",
               "CNNDetection/model/B_C1_plan3_epoch_10_acc_98.5.pth",
               ]
...
with torch.no_grad():
    for images, labels in test_loader:
        ...
        # # 进行模型集成预测 (多数投票)
        # all_predictions = torch.zeros(len(models), images.size(0)).long().cuda()  # [模型数量, 批次大小]
        # for i, model in enumerate(models):
        #     output = model(images)  # 每个模型的输出
        #     probs = torch.nn.functional.softmax(output, dim=1)  # 转换为概率
        #     # print(f'probs_{i}={probs}')
        #     fake_probs = probs[:, 1]  # 获取fake类别的概率
        #     # print(f'fake_probs_{i}={fake_probs}')
        #     all_predictions[i] = (fake_probs >= threshold).long()  # 根据阈值判断该模型的预测标签
        #     # print(f'all_predictions[{i}]={all_predictions[i]}')
        # # 对所有模型的预测结果进行多数投票
        # # print(f'all_predictions={all_predictions}')
        # predicted_labels, _ = torch.mode(all_predictions, dim=0)  # 取每个样本多数投票的结果作为最终预测
