...
def binary_dataset(opt, root):
    if opt.isTrain:
        ...
        crop_func = transforms.Lambda(lambda img: img)  # 返回原图
    else:
    ...
    dset = datasets.ImageFolder(  # datasets.ImageFolder 是 PyTorch 中的一个数据集类，专门用于从目录中加载图片数据。这个类假设 root 目录中包含子文件夹，每个子文件夹的名称作为类标签，子文件夹中的图片被视为对应类别的数据。
            root,
            transforms.Compose([
                # rz_func,
                # crop_func,
    ...
    print('dset:',dset)
    print('dset.class_to_idx:',dset.class_to_idx)
